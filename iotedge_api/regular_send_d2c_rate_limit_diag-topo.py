...
OR_ID_RANGE = list(range(1,26))
DEVICE_ID_RANGE = list(range(1,41))
...
URL = f'http://{IP_ADDRESS}:80/api/v1/devices/messages'
#URL = f'http://10.98.0.207:80/api/v1/devices/messages'

#@limits(calls=MAX_CALLS_IN_DURATION, period=DURATION_IN_SEC)
@sleep_and_retry
@limits(calls=1000, period=45)
def access_rate_limited_api(count):
    resp = send_d2c_request(MESSAGE_TYPE,URL)
    return count,resp
